// CommentsContainer component - it shows all comments of the video

// dummy hard-coded Comments data
const commentsData = [
  {
    name: "MR A",
    text: "Example comment 1 ...",
    replies: [
      {
        name: "User A",
        text: "Example comment 1 ...",
        replies: [
          { name: "P", text: "Example comment 6 ...", replies: [] },
          { name: "Q", text: "Example comment 7 ...", replies: [] },
          { name: "R", text: "Example comment 8 ...", replies: [] },
          { name: "S", text: "Example comment 9 ...", replies: [] },
        ],
      },
      { name: "User B", text: "Example comment 9 ...", replies: [] },
      { name: "User C", text: "Example comment 5 ...", replies: [] },
    ],
  },
  {
    name: "MR B",
    text: "Example comment 2 ...",
    replies: [
      {
        name: "User K",
        text: "Example comment 1 ...",
        replies: [
          { name: "X", text: "Example comment 12 ...", replies: [] },
          { name: "Y", text: "Example comment 13 ...", replies: [] },
          { name: "Z", text: "Example comment 14 ...", replies: [] },
          { name: "O", text: "Example comment 15 ...", replies: [] },
        ],
      },
      { name: "User B", text: "Example comment 1 ...", replies: [] },
      { name: "User C", text: "Example comment 1 ...", replies: [] },
    ],
  },
  { name: "MR C", text: "Example comment 3 ...", replies: [] },
  { name: "MR D", text: "Example comment 4 ...", replies: [] },
  { name: "MR E", text: "Example comment 5 ...", replies: [] },
];

// CommentCard component
const Comment = (props) => {
  const { data } = props;
  const { name, text, replies } = data;

  return (
    <div className="flex gap-2  p-2 my-2 bg-gray-50 shadow-sm rounded-lg">
      <img
        src="data:image/png;base64,"
        alt="user"
        className="h-12 w-12"
      />
      <div className="">
        <p>{name}</p>
        <p>{text}</p>
      </div>
    </div>
  );
};

// CommentCardsList component
const CommentsList = (props) => {
  const { comments } = props;

  // Disclaimer: Don't use indexs as keys
  return comments.map((comment, index) => (
    <Comment data={comment} key={index} />
  ));
};

// CommentsContainer component
const CommentsContainer = () => {
  return (
    <div className="m-5 p-2">
      <h1 className="text-2xl font-bold"> Comments: </h1>

      {/* render CommentsList component */}
      <CommentsList comments={commentsData} />
    </div>
  );
};

export default CommentsContainer;
